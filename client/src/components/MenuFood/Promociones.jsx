...
export default function Promociones({ setCartItems, setShowSummary, setQuantities, quantities }) {
  ...
  const piqueos = restaurantdetails?.Menus.filter(menu => menu.category.includes("Promociones") && menu.stock === true)
      ...
      {piqueos?.length > 0 ? (
        <div className={styles.menufood_container}>
          <div className={styles.container_bg_none}>
            <h1 className={styles.title_Carrusel2}>Promociones</h1>
            <Splide
              options={{
                perPage: 4,
                perMove: 1,
